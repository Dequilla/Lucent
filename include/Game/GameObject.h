    ...
    namespace game
    {
        ...
        class GameObject
        {
            protected:
                ...
                GameObject *m_parent =
                  nullptr; // Also have parent's components creating recursion?
                           // Only problem: GameObjects inside GameObjects being
                           // positioned based on its parent
                std::vector<BaseComponent *>
                  m_components; // TODO: Give pointer of this to components so
                                // all components can access its hosts(game
                                // objects) components (eg: modelComps can
                                // access transforComps) And we avoid circular
                                // dependencies
            public:
                /**
                ...
                /**
                 * \brief Recursive function that finds the object with name and
                 * removes it
                 *
                 ...
                 *
                 * It looks through it's children to find one with the matching
                 * name, if it can't find it it checks their children and so on.
                 *
                 * @param name The name of the object to look for
                 * @return Pointer to that game object, or nullptr if it can't
                 * be found
                 */
                 ...
                 *
                 * @param type Type string as represented by a components
                 * getType() function
                 * @return A component of the type specified, nullptr if none
                 * exist
                 */
                template<class T>
        ...
        template<class T>
        inline std::vector<T *> lu::game::GameObject::getComponentsOfType(
          std::string type)
        {
            int count = 0;
            std::vector<T *> result;
