    ...
    namespace game
    {
        ...
        class BaseComponent
        {
            protected:
                 ...
                 *
                 * Used to access other components on the host object such as
                 * TransformComponents etc Since it is a pointer, if the host
                 * components change, it is updated here too
                 */
            ...
            public:
                 ...
                 *
                 * T needs to be the class you are getting as a type as it
                 * converts it to that type Returns a vector with the first
                 * element being a nullptr if it can't find any
                 */
                template<class T>
        ...
        template<class T>
        inline std::vector<T *>
        lu::game::BaseComponent::getHostComponentsOfType(std::string type)
        {
            int count = 0;
            std::vector<T *> result;
            ...
            for (unsigned int i = 0; i < m_hostComponents->size(); i++)
            {
                ...
                {
                    result.push_back(
                      dynamic_cast<T *>(m_hostComponents->at(i)));
                    count++;
                }
            }
