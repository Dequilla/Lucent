...
public:
   ...
   * @param flags WindowFlags describing how the window should be created
   */
  void create(std::string title, int posx, int posy, int width, int height,
              unsigned int flags = WINDOW_SHOWN);
  ...
  void display(); /**< Changes buffer behind the scene essentially displaying
   ...
   *
   * Essentially tell the window to hide the cursor and keep it inside the
   ...
   *
   * @param type Should be either WINDOW_FULLSCREEN, WINDOW_FULLSCREEN_DESKTOP
   * or 0 to set back to windowed
   */
  void setWindowFullscreen(unsigned int type);

  glm::vec2
  getSize(); /**< Returns a vector with the width and height of the window */
};
