...
namespace lu { namespace core {
	...
	*/
	enum ConsoleColor
	{
		CONSOLE_RESET,	/**< Used to reset to the standard colors of the console */
	...
	inline void changeConsoleColor(ConsoleColor foreground, ConsoleColor background = CONSOLE_BLACK)
	{
...
#ifdef __linux__
	// TODO: Find way to force doxygen to document these functions aswell
	inline void changeConsoleColor(ConsoleColor foreground, ConsoleColor background)
	{
		std::string fColor = "";
		std::string bColor = "";
	...
	*/
	inline void printWithColors(std::string output, ConsoleColor foreground, ConsoleColor background = CONSOLE_BLACK)
	{
		changeConsoleColor(foreground, background);

		std::cout << output << std::endl;

		changeConsoleColor(CONSOLE_RESET, CONSOLE_RESET); // reset to default otherwise the colorchange persists
	}
