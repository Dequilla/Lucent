...
std::vector<std::pair<lu::graphics::ShaderProperties, lu::graphics::Shader>>
  lu::graphics::ShaderLoader::m_loadedShaders;

lu::graphics::Shader
lu::graphics::ShaderLoader::loadFromSource(const GLchar *vertexSource,
...
lu::graphics::Shader
lu::graphics::ShaderLoader::loadShader(ShaderProperties properties,
                                       bool withLights)
{
    ...
    if (!alreadyExists(properties))
    {
        ...
        if (withLights)
        {
            ...
            fragmentCode = core::string::replace(
              fragmentCode,
              "${NUM_DIR_LIGHTS}",
              std::to_string(
                lu::core::Application::getInstance().maxDirLights));

            // Comment out lightingloop if we have no lights
            fragmentCode = core::string::replace(
              fragmentCode,
              "${HAS_DIR_LIGHT1}",
              (lu::core::Application::getInstance().maxDirLights > 0) ? ""
                                                                      : "//");
            fragmentCode = core::string::replace(
              fragmentCode,
              "${HAS_DIR_LIGHT2}",
            ...
            fragmentCode = core::string::replace(
              ...
              "${NUM_POINT_LIGHTS}",
              std::to_string(
                lu::core::Application::getInstance().maxPointLights));
            // Comment out lightingloop if we have no lights
            fragmentCode = core::string::replace(
              fragmentCode,
              "${HAS_POINT_LIGHT1}",
              (lu::core::Application::getInstance().maxPointLights > 0) ? ""
                                                                        : "//");
            fragmentCode = core::string::replace(
              fragmentCode,
              "${HAS_POINT_LIGHT2}",
            ...
            fragmentCode = core::string::replace(
              ...
              "${NUM_SPOT_LIGHTS}",
              std::to_string(
                lu::core::Application::getInstance().maxSpotLights));
            // Comment out lightingloop if we have no lights
            fragmentCode = core::string::replace(
              fragmentCode,
              "${HAS_SPOT_LIGHT1}",
              (lu::core::Application::getInstance().maxSpotLights > 0) ? ""
                                                                       : "//");
            fragmentCode = core::string::replace(
              fragmentCode,
              "${HAS_SPOT_LIGHT2}",
        ...
        m_loadedShaders.emplace_back(std::make_pair(
          properties, loadFromSource(vertexSource, fragmentSource)));
    }
...
bool
lu::graphics::ShaderLoader::alreadyExists(ShaderProperties properties)
{
    for (auto &shader : m_loadedShaders)
    {
        if (shader.first == properties)
