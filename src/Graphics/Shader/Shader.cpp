...
std::unordered_map<std::string, ce::graphics::Shader> ce::graphics::Shader::m_loadedShaders;

ce::graphics::Shader::Shader(const GLchar* vertexPath, const GLchar* fragmentPath)
{
	std::string allPaths = (std::string(vertexPath) + std::string(fragmentPath));

	// If already loaded and compiled
	if (m_loadedShaders.count(allPaths) > 0)
	{
		this->programID = m_loadedShaders.at(allPaths).programID;
	}
	else // If not loaded load
	{
		...
		// Add itself to m_loadedShaders
		Shader shader;
		shader.programID = this->programID;
		m_loadedShaders.emplace(allPaths, shader);
