...
lu::game::ExampleGameMode::init()
{
    ...
    lu::graphics::Model grassyModel =
      loader.loadModel("Resources/Models/TestTerrain/TestTerrain.obj");
    ...
    lu::graphics::Model nanosuit =
      loader.loadModel("Resources/Models/Nanosuit/nanosuit.obj");
    ...
    m_camera.camera()->setProjection(
      80.f, (float)size.x / (float)size.y, 0.1f, 100.0f);
    ...
    // Input
    m_input.bindAxisEvent(
      lu::core::M_MOTION_XREL, this, &ExampleGameMode::cameraYaw);
    m_input.bindAxisEvent(
      lu::core::M_MOTION_YREL, this, &ExampleGameMode::cameraPitch);

    m_input.bindButtonEvent(lu::core::K_w, this, &ExampleGameMode::moveForward);
    m_input.bindButtonEvent(
      lu::core::K_s, this, &ExampleGameMode::moveBackward);
...
void
lu::game::ExampleGameMode::begin()
{
    glm::vec2 size = lu::core::Application::getScreenBufferSize();
    m_renderer.setScreenBufferSize(size.x, size.y);

    m_renderer.begin();
    m_renderer.beginScene(m_camera.camera()->getViewMatrix(),
                          m_camera.camera()->getProjectionMatrix());

    GameMode::begin();
}

void
lu::game::ExampleGameMode::tick(float dt)
{
    ...
    m_spotLight.light()->position =
      glm::vec3(-m_camera.transform()->getPosition().x,
                -m_camera.transform()->getPosition().y,
                -m_camera.transform()->getPosition().z);
...
void
lu::game::ExampleGameMode::draw()
{
    GameMode::draw(&m_renderer);
}

void
lu::game::ExampleGameMode::end()
{
    m_renderer.endScene();
    m_renderer.end();
...
void
lu::game::ExampleGameMode::cameraYaw(float axis)
{
    m_orientYaw += axis * 0.5f;
}

void
lu::game::ExampleGameMode::cameraPitch(float axis)
{
...
void
lu::game::ExampleGameMode::moveLeft(bool pressed)
{
    m_moveLeft = pressed;
}

void
lu::game::ExampleGameMode::moveRight(bool pressed)
{
    m_moveRight = pressed;
}

void
lu::game::ExampleGameMode::moveForward(bool pressed)
{
    m_moveForward = pressed;
}

void
lu::game::ExampleGameMode::moveBackward(bool pressed)
{
    m_moveBackward = pressed;
}

void
lu::game::ExampleGameMode::movePLightF(bool pressed)
{
...
void
lu::game::ExampleGameMode::movePLightB(bool pressed)
{
    m_pointLights[0].light()->position += glm::vec3(0.0f, 0.0f, 0.2f);
    m_nanosuit.transform()->move(glm::vec3(0.0f, 0.0f, 0.2f));
}
...
void
lu::game::ExampleGameMode::movePLightL(bool pressed)
{
...
void
lu::game::ExampleGameMode::movePLightR(bool pressed)
{
    m_pointLights[0].light()->position += glm::vec3(0.2f, 0.0f, 0.0f);
    m_nanosuit.transform()->move(glm::vec3(0.2f, 0.0f, 0.0f));
}