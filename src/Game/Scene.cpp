...
lu::game::RootObject *
lu::game::Scene::getRootObject()
{
    return &m_rootObject;
}

lu::game::GameObject *
lu::game::Scene::getGameObjectByName(std::string name)
{
    return m_rootObject.getGameObjectByName(name);
}

lu::game::GameObject *
lu::game::Scene::addGameObject(std::string nameOfParent, GameObject *object)
{
    ...
    // If not root, go through all objects and check for the parent, then add
    // it, then return
...
void
lu::game::Scene::tick(float dt)
{
    m_rootObject.tick(dt);
}

void
lu::game::Scene::draw(lu::graphics::Renderer3D *renderer)
{
