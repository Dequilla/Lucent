...
int main(int argc, char *argv[]) {
  ...
  lu::core::Window window("Lucent", lu::core::WINDOWPOS_CENTERED,
                          lu::core::WINDOWPOS_CENTERED, SCREEN_WIDTH,
                          SCREEN_HEIGHT, lu::core::WINDOW_RESIZABLE);
    ...
    while (SDL_PollEvent(&e) != 0) {
      ...
      if (e.type == SDL_KEYDOWN) {
        ...
        if (e.key.keysym.sym == SDLK_F11) {
          // Buggy on linux it seems - probably happens too fast, so it
          // instantly switches back
          if (fullscreen == false) {
            window.setWindowFullscreen(lu::core::WINDOW_FULLSCREEN_DESKTOP);
            lu::core::Application::setScreenBufferSize(window.getSize().x,
                                                       window.getSize().y);
            fullscreen = true;
          } else {
