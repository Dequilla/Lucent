...
lu::core::Window::Window(std::string title, int posx, int posy, int width,
                         int height, unsigned int flags) {
  create(title, posx, posy, width, height, flags);
}

lu::core::Window::~Window() { destroyWindow(); }

void lu::core::Window::create(std::string title, int posx, int posy, int width,
                              int height, unsigned int flags) {
  initVideoComponents();
  ...
  // Create the window
  m_window = SDL_CreateWindow(title.c_str(), posx, posy, width, height,
...
void lu::core::Window::display() { SDL_GL_SwapWindow(m_window); }
